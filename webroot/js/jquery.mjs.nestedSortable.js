...
(function(factory) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		options: {
			...
			// IMPORTANT FIX:
			// expandOnHover must be a NUMBER (delay in ms). The previous script defined it twice
			// (number + function), which broke hover-expand timing.
			expandOnHover: 700,

			// OPTIONAL HOOK (keeps your custom behavior without breaking expandOnHover delay)
			expandOnHoverCheck: function() { return true; },
		...
		_mouseDrag: function(event) {
			...
			$(newList).addClass(o.addListClass);
			...
			if (parentItem != null &&
				...
				// IMPORTANT FIX: ensure parentItem is a non-empty jQuery object
		...
		_intersectsWithSides: function(item) {
			...
			// IMPORTANT FIX:
			// Previously this called this.options.expandOnHover(item), but expandOnHover is a NUMBER (delay).
			// If you need a per-cycle hook, use expandOnHoverCheck.
			if (typeof this.options.expandOnHoverCheck === "function") {
				this.options.expandOnHoverCheck(item);
			}
