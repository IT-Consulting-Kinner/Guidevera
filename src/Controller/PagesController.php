...
class PagesController extends AppController
{
    ...
    private function _showNumbering(): bool
    {
    ...
    {
        $locale = $this->_locale();
        $numberedPages = PagesService::getNumberedPages($this->_showNumbering());
        $showRoot = Configure::read('Manual.showNavigationRoot') ?? true;

        if (!$id && !empty($numberedPages)) {
            foreach ($numberedPages as $i => $p) {
                if ($i === 0 && !$showRoot && !$this->isLoggedIn()) continue;
                if (($p->status ?? $p['status'] ?? '') === 'active') { $id = $p->id ?? $p['id']; break; }
            }
            if (!$id && !empty($numberedPages)) $id = $numberedPages[0]->id ?? $numberedPages[0]['id'];
        }
        ...
        if ($id) {
            try {
                ...
                $page = $this->_applyTranslation($page, $locale);
        ...
        $this->set(compact('page', 'allPages', 'numberedPages', 'nav', 'id', 'ssrNavHtml', 'ssrTree', 'feedbackSummary'));
    }
    ...
    public function getTree(): ?\Cake\Http\Response
    {
        ...
        $pages = PagesService::getNumberedPages($this->_showNumbering());
    ...
    public function show(): ?\Cake\Http\Response
    {
        ...
        $locale = $this->request->getData('locale', $this->_locale());
        ...
        try {
            ...
            $numbered = PagesService::getNumberedPages($this->_showNumbering());
            foreach ($numbered as $np) {
                if (($np->id ?? $np['id'] ?? 0) == $id) { $page->title = $np->title ?? $np['title'] ?? $page->title; break; }
            }
            ...
            return $this->jsonSuccess([
                ...
                'breadcrumbs' => (Configure::read('Manual.enableBreadcrumbs') ?? true) ? PagesService::buildBreadcrumbs($id, $numbered) : [],
                'nav' => (Configure::read('Manual.enablePrevNext') ?? true) ? PagesService::calculateNavigation($id, $numbered) : [],
            ]);
        } catch (\Exception $e) {
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!$this->requireRole(self::ROLE_EDITOR)) return $this->response;
        $id = (int)$this->request->getData('id');
        $locale = $this->request->getData('locale', $this->_locale());
        if (!$id) return $this->jsonError('invalid_id');
        try {
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!$this->requireRole(self::ROLE_CONTRIBUTOR)) return $this->response;
        ...
        try {
            if ($this->Pages->save($page)) {
                ...
                $this->sendNotification('Page created', "New page #{$page->id} created by " . ($user['fullname'] ?? 'Unknown'));
                return $this->jsonSuccess(['intId' => $page->id]);
            }
        } catch (\Exception $e) { Log::error('Page create failed: ' . $e->getMessage()); }
    ...
    public function save(): ?\Cake\Http\Response
    {
        ...
        try {
            ...
            if ($locale === $defaultLocale) {
                ...
                if (!$this->Pages->save($page)) return $this->jsonError('can_not_save_item');
            } else {
                $trans = $this->fetchTable('PageTranslations');
                $existing = $trans->find()->where(['page_id' => $id, 'locale' => $locale])->first();
                $data = ['title' => $title, 'description' => $description, 'content' => $content, 'modified_by' => $user['id'] ?? 0];
                if ($existing) { $trans->save($trans->patchEntity($existing, $data)); }
                else { $trans->save($trans->newEntity(array_merge($data, ['page_id' => $id, 'locale' => $locale]))); }
                $this->Pages->updateAll(['modified_by' => $user['id'] ?? 0], ['id' => $id]);
            }

            PagesService::invalidateCache();
            $this->_saveKeywords($id, $this->request->getData('keywords', ''));
            $this->audit('page_save', 'page', $id, "'{$title}' saved by " . ($user['fullname'] ?? ''));
            $this->sendNotification('Page updated', "Page '{$title}' (#{$id}) updated by " . ($user['fullname'] ?? 'Unknown'));
            \App\Service\WebhookService::fire('page.updated', ['page_id' => $id, 'title' => $title]);
            // Notify subscribers
            if (Configure::read('Manual.enableSubscriptions') ?? false) {
                $this->_notifySubscribers($id, "Page '{$title}' has been updated.");
            }
            // Mark translations as potentially stale
            if ($locale === $defaultLocale && (Configure::read('Manual.enableTranslations') ?? false)) {
                $this->fetchTable('PageTranslations')->updateAll(['base_modified' => new \Cake\I18n\DateTime()], ['page_id' => $id]);
            }
            return $this->jsonSuccess(['intAffectedRows' => 1]);
        } catch (\Exception $e) { Log::error('Page save failed: ' . $e->getMessage()); }
    ...
    public function delete(): ?\Cake\Http\Response
    {
        ...
        if (!$this->requireRole(self::ROLE_CONTRIBUTOR)) return $this->response;
        $id = (int)$this->request->getData('id');
        if ($this->Pages->findChildrenOf($id)->count() > 0) return $this->jsonError('has_child');
        try {
            ...
            return $this->jsonSuccess(['intAffectedRows' => 1]);
        } catch (\Exception $e) { Log::error('Page delete failed: ' . $e->getMessage()); }
    ...
    public function setStatus(): ?\Cake\Http\Response
    {
        ...
        if (!in_array($status, ['active', 'inactive'])) return $this->jsonError('invalid_status');
    ...
    public function updateOrder(): ?\Cake\Http\Response
    {
        ...
        if (!empty($arr['list'])) {
            foreach ($arr['list'] as $pageId => $parentId) {
                $this->Pages->updateAll(['position' => $count, 'parent_id' => $parentId === 'null' ? null : $parentId], ['id' => (int)$pageId]);
                $count++;
            }
        }
        PagesService::invalidateCache();
    ...
    public function updateParent(): ?\Cake\Http\Response
    {
        ...
        $this->Pages->updateAll(['parent_id' => ($parentId === 'null' || $parentId === '0') ? null : $parentId], ['id' => $id]);
        PagesService::invalidateCache();
        return $this->jsonSuccess(['intAffectedRows' => 1]);
    }
    ...
    {
        $this->autoRender = false;
        if (!$this->isLoggedIn()) return $this->jsonError('not_authenticated');
        $numbered = PagesService::getNumberedPages($this->_showNumbering());
        $pages = [];
    ...
    public function search(): ?\Cake\Http\Response
    {
        ...
        if (empty($search)) return $this->jsonSuccess(['results' => []]);
        $filters = [
        ...
        if (empty($result['results'])) $this->audit('search_no_results', 'search', 0, $search);
    ...
    public function uploadMedia(): ?\Cake\Http\Response
    {
        ...
        if (!$file) return $this->jsonError('upload_failed');
    ...
    {
        $this->set('pages', $this->Pages->find()->where(['status' => 'active', 'deleted_at IS' => null])->orderBy(['position' => 'ASC'])->all());
    }

    public function printPage(?int $id = null): void
    {
        if (!(Configure::read('Manual.enablePrint') ?? false)) { $this->redirect('/'); return; }
        $this->viewBuilder()->disableAutoLayout();
        if (!$id) $id = (int)$this->request->getParam('id');
        if (!$id) { $this->set('error', 'Page not found'); return; }
        $result = $this->PageContent->loadPrintPage($id, $this->isLoggedIn());
        if ($result === null) { $this->set('error', 'Page not found'); return; }
        $this->set($result);
    }
    ...
    {
        if (!$this->isLoggedIn()) { $this->redirect('/user/login'); return; }
        ...
        // Quality metrics
        $stalePages = $this->Pages->find()->where(['deleted_at IS' => null, 'modified <' => new \Cake\I18n\DateTime('-12 months')])->count();
        $noDescription = $this->Pages->find()->where(['deleted_at IS' => null, 'OR' => ['description IS' => null, 'description' => '']])->count();
        ...
        // My drafts
        $myDrafts = $this->Pages->find()->where(['deleted_at IS' => null, 'workflow_status' => 'draft', 'modified_by' => $userId])
            ->orderBy(['modified' => 'DESC'])->limit(10)->all()->toArray();
        ...
        $this->set(compact('recentlyEdited', 'recentlyCreated', 'trashCount', 'totalPages', 'pendingFeedback', 'stalePages', 'noDescription', 'reviewQueue', 'searchMisses', 'myDrafts', 'myReviews'));
    }
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!$this->requireRole(self::ROLE_ADMIN)) return $this->response;
        $id = (int)$this->request->getData('id', 0);
        if ($id) {
            // Purge single item
    ...
    {
        $this->autoRender = false;
        if (!(Configure::read('Manual.enableMarkdownExport') ?? false)) return $this->jsonError('feature_disabled');
        ...
        try {
            ...
            return $this->response->withType('text/markdown')
                ->withHeader('Content-Disposition', 'attachment; filename="' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $page->title) . '.md"')
                ->withStringBody($md);
        } catch (\Exception $e) { return $this->jsonError('export_failed'); }
    }

    public function exportPdf(): ?\Cake\Http\Response
    {
        $this->autoRender = false;
        if (!(Configure::read('Manual.enablePdfExport') ?? false)) return $this->jsonError('feature_disabled');
        $id = (int)$this->request->getQuery('id', 0);
        if (!$id) return $this->jsonError('invalid_id');
        try {
            $page = $this->Pages->get($id, contain: ['CreatedByUsers', 'ModifiedByUsers']);
            $html = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{font-family:sans-serif;max-width:800px;margin:auto;padding:2rem}h1{border-bottom:2px solid #333}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ccc;padding:0.5rem}</style></head><body>';
            ...
            if ($wk && file_exists($wk)) {
                exec(escapeshellarg($wk) . ' --quiet ' . escapeshellarg($tmpHtml) . ' ' . escapeshellarg($tmpPdf) . ' 2>&1');
                if (file_exists($tmpPdf)) {
                    $pdf = file_get_contents($tmpPdf);
                    @unlink($tmpHtml); @unlink($tmpPdf);
                    return $this->response->withType('application/pdf')
                        ->withHeader('Content-Disposition', 'attachment; filename="' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $page->title) . '.pdf"')
                        ->withStringBody($pdf);
                }
            }
            // Fallback: serve HTML
            ...
            return $this->response->withType('text/html')
                ->withHeader('Content-Disposition', 'attachment; filename="' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $page->title) . '.html"')
                ->withStringBody($html);
        } catch (\Exception $e) { return $this->jsonError('export_failed'); }
    }
    ...
    public function stats(): ?\Cake\Http\Response
    {
        ...
        $recentActivity = $this->Pages->find()->where(['deleted_at IS' => null])
            ->orderBy(['modified' => 'DESC'])->limit(10)->select(['id', 'title', 'modified', 'modified_by'])->all()->toArray();
        ...
        return $this->jsonSuccess([
            ...
            'topViewed' => array_map(fn($p) => ['id' => $p->id, 'title' => $p->title, 'views' => $p->views], $topViewed),
            'recentActivity' => array_map(fn($p) => ['id' => $p->id, 'title' => $p->title, 'modified' => $p->modified->format('d.m.Y H:i')], $recentActivity),
            'feedback' => $fbStats,
            'searchMisses' => array_map(fn($l) => ['term' => $l->details, 'date' => $l->created->format('d.m.Y')], $searchMisses),
        ]);
    }
    ...
    public function auditLog(): ?\Cake\Http\Response
    {
        $this->autoRender = false;
        if (!$this->requireRole(self::ROLE_ADMIN)) return $this->response;
        if (!(Configure::read('Manual.enableAuditLog') ?? false)) return $this->jsonError('feature_disabled');
    ...
    public function setWorkflowStatus(): ?\Cake\Http\Response
    {
        ...
        if (!in_array($ws, $allowed)) return $this->jsonError('invalid_workflow_status');
    ...
    public function relatedPages(): ?\Cake\Http\Response
    {
        ...
        $myTags = $this->fetchTable('PageTags')->find()
            ->where(['page_id' => $pageId])->all()->extract('tag')->toArray();
        if (empty($myTags)) return $this->jsonSuccess(['related' => []]);
    ...
    public function qualityReport(): ?\Cake\Http\Response
    {
        ...
        if (!$data) {
            ...
            foreach ($pages as $p) {
                if (empty(trim($p->description ?? ''))) $noDesc++;
                if ($p->modified && $p->modified->wasWithinLast('12 months') === false) $stale++;
                if (strlen(trim(strip_tags($p->content ?? ''))) < 10) $empty++;
            }
            $data = [
                'timestamp' => date('Y-m-d H:i:s'),
                'summary' => ['stale' => $stale, 'noDescription' => $noDesc, 'emptyContent' => $empty, 'totalIssues' => $stale + $noDesc + $empty],
                'issues' => [],
                'orphaned' => [],
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!(Configure::read('Manual.enableSubscriptions') ?? false)) return $this->jsonError('feature_disabled');
        if (!$this->isLoggedIn()) return $this->jsonError('not_authenticated');
        $pageId = (int)$this->request->getData('page_id', 0);
        if (!$pageId) return $this->jsonError('invalid_id');
        ...
        if ($existing) { $tbl->delete($existing); return $this->jsonSuccess(['subscribed' => false]); }
    ...
    {
        $this->autoRender = false;
        if (!$this->isLoggedIn()) return $this->jsonSuccess(['subscribed' => false]);
        $pageId = (int)$this->request->getData('page_id', 0);
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!(Configure::read('Manual.enableAcknowledgements') ?? false)) return $this->jsonError('feature_disabled');
        if (!$this->isLoggedIn()) return $this->jsonError('not_authenticated');
        $pageId = (int)$this->request->getData('page_id', 0);
        if (!$pageId) return $this->jsonError('invalid_id');
    ...
    {
        $this->autoRender = false;
        if (!(Configure::read('Manual.enableInlineComments') ?? false)) return $this->jsonError('feature_disabled');
    ...
    public function addInlineComment(): ?\Cake\Http\Response
    {
        ...
        $entity = $tbl->newEntity([
            ...
            'anchor' => $this->request->getData('anchor', ''), 'comment' => mb_substr($this->request->getData('comment', ''), 0, 2000),
        ]);
        if ($tbl->save($entity)) return $this->jsonSuccess(['id' => $entity->id]);
    ...
    public function resolveInlineComment(): ?\Cake\Http\Response
    {
        ...
        if (!$comment) return $this->jsonError('not_found');
        $userId = $this->currentUser()['id'] ?? 0;
    ...
    public function analytics(): ?\Cake\Http\Response
    {
        $this->autoRender = false;
        if (!$this->requireRole(self::ROLE_ADMIN)) return $this->response;
        if (!(Configure::read('Manual.enableContentAnalytics') ?? false)) return $this->jsonError('feature_disabled');
        ...
        foreach ($badFeedback as $f) {
            ...
            if ($page) $badPages[] = ['id' => $page->id, 'title' => $page->title, 'negativeCount' => (int)$f->neg];
        }
        ...
        foreach ($freq as $f) {
            $page = $this->Pages->find()->select(['id', 'title'])->where(['id' => $f->page_id])->first();
            if ($page) $freqPages[] = ['id' => $page->id, 'title' => $page->title, 'revisionCount' => (int)$f->cnt];
        }

        return $this->jsonSuccess([
            'topViewed' => array_map(fn($p) => ['id' => $p->id, 'title' => $p->title, 'views' => $p->views], $topViewed),
            'leastViewed' => array_map(fn($p) => ['id' => $p->id, 'title' => $p->title, 'views' => $p->views], $leastViewed),
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!(Configure::read('Manual.enableImport') ?? false)) return $this->jsonError('feature_disabled');
        if (!$this->requireRole(self::ROLE_CONTRIBUTOR)) return $this->response;
        ...
        if ($file && $file->getError() === UPLOAD_ERR_OK) {
            ...
            if (empty($title)) $title = pathinfo($file->getClientFilename(), PATHINFO_FILENAME);
        }

        if (empty($content)) return $this->jsonError('no_content');
    ...
    {
        $this->autoRender = false;
        if (!(Configure::read('Manual.enableTranslations') ?? false)) return $this->jsonError('feature_disabled');
        $locales = Configure::read('Manual.contentLocales') ?? ['en'];
        $defaultLocale = Configure::read('Manual.defaultLocale') ?? 'en';
        $pages = $this->Pages->find()->select(['id', 'title', 'modified'])->where(['deleted_at IS' => null])->orderBy(['position' => 'ASC'])->all();
        $trans = $this->fetchTable('PageTranslations');
        ...
        foreach ($pages as $p) {
            ...
            foreach ($locales as $loc) {
                ...
                if (!isset($existing[$loc])) $missing[] = $loc;
            }

            $staleLocales = [];
            foreach ($existing as $loc => $info) {
                if (!empty($info['stale'])) $staleLocales[] = $loc;
            }
    ...
    {
        $this->request->allowMethod(['post']); $this->autoRender = false;
        if (!(Configure::read('Manual.enableReviewProcess') ?? false)) return $this->jsonError('feature_disabled');
        if (!$this->requireRole(self::ROLE_CONTRIBUTOR)) return $this->response;
        $pageId = (int)$this->request->getData('page_id', 0);
        $reviewerId = (int)$this->request->getData('reviewer_id', 0);
        if (!$pageId || !$reviewerId) return $this->jsonError('invalid_data');

        $tbl = $this->fetchTable('PageReviews');
        $review = $tbl->newEntity(['page_id' => $pageId, 'reviewer_id' => $reviewerId, 'status' => 'pending', 'comment' => '']);
        if ($tbl->save($review)) {
            $this->Pages->updateAll(['workflow_status' => 'review'], ['id' => $pageId]);
            $reviewer = $this->fetchTable('Users')->find()->select(['fullname', 'email'])->where(['id' => $reviewerId])->first();
            $page = $this->Pages->find()->select(['title'])->where(['id' => $pageId])->first();
            $this->sendNotification("Review requested: {$page->title}", "You have been assigned to review '{$page->title}'.");
    ...
    public function reviewDecision(): ?\Cake\Http\Response
    {
        ...
        if (!$reviewId || !in_array($decision, ['approved', 'rejected', 'revision_needed'])) return $this->jsonError('invalid_data');
        if ($decision === 'rejected' && empty(trim($comment))) return $this->jsonError('comment_required');

        $tbl = $this->fetchTable('PageReviews');
        $review = $tbl->find()->where(['id' => $reviewId])->first();
        if (!$review) return $this->jsonError('not_found');
        ...
        if ($review->status !== 'pending') return $this->jsonError('review_already_decided');
        ...
        if ($decision === 'approved') $this->Pages->updateAll(['status' => 'active'], ['id' => $review->page_id]);
    ...
    private function _notifySubscribers(int $pageId, string $message): void
    {
        try {
            ...
            $userId = $this->currentUser()['id'] ?? 0;
            foreach ($subs as $s) {
                if ($s->user_id === $userId) continue; // Don't notify self
                if (!empty($s->user->email)) {
                    $this->sendNotification('Page update notification', $message);
                }
            }
        } catch (\Exception $e) { Log::error('Subscriber notification failed: ' . $e->getMessage()); }
    }

    private function _saveKeywords(int $pageId, string $keywords): void
    {
        $tbl = $this->fetchTable('Pagesindex');
        $tbl->deleteAll(['page_id' => $pageId]);
        if (empty(trim($keywords))) return;
        foreach (array_map('trim', explode(',', $keywords)) as $kw) {
            if (!empty($kw)) $tbl->save($tbl->newEntity(['keyword' => $kw, 'page_id' => $pageId]));
        }
    }

    private function _applyTranslation($page, string $locale): object
    {
        $defaultLocale = Configure::read('Manual.defaultLocale') ?? 'en';
        if ($locale === $defaultLocale || !(Configure::read('Manual.enableTranslations') ?? false)) return $page;
        try {
        ...
        } catch (\Exception $e) { Log::error('Translation load failed: ' . $e->getMessage()); }
        return $page;
    }
}
