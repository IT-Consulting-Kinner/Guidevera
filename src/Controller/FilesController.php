...
class FilesController extends AppController
{
    ...
    public function upload(): ?\Cake\Http\Response
    {
        ...
        if (!$this->hasRole(self::ROLE_CONTRIBUTOR)) return $this->jsonError('not_authenticated');
        ...
        $filename = \App\Service\UploadService::resolveConflict(
            basename($file->getClientFilename()), $this->mediaDir
        );
        ...
        return $this->jsonSuccess([
            ...
            'filename' => $filename,
            'hash' => md5($filename),
            'size' => $this->_formatFilesize(filesize($target)),
    ...
    public function delete(): ?\Cake\Http\Response
    {
        ...
        if (empty($filename) || $filename[0] === '.') return $this->jsonError('invalid_filename');

        $path = $this->mediaDir . $filename;
        if (!file_exists($path)) return $this->jsonError('file_not_found');
    ...
    {
        $this->autoRender = false;
        if (!$this->hasRole(self::ROLE_EDITOR)) return $this->jsonError('not_authenticated');

        $files = [];
        if (is_dir($this->mediaDir)) {
            foreach (scandir($this->mediaDir) as $entry) {
                if ($entry[0] === '.' || !is_file($this->mediaDir . $entry)) continue;
                $files[] = ['name' => $entry];
            }
        }
    ...
    {
        $files = [];
        if (!is_dir($this->mediaDir)) return $files;
        ...
        foreach (scandir($this->mediaDir) as $entry) {
            ...
            $files[] = [
                'name' => $entry,
                'size' => $this->_formatFilesize(filesize($path)),
                'date' => date('d.m.Y H:i', filemtime($path)),
                'views' => $this->_countViews($entry),
            ];
        }
        return $files;
    }
     ...
     */
    private function _countViews(string $filename): int
    {
        $logFile = $this->mediaDir . '.' . $filename;
