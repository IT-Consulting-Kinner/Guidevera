...
<head>
    ...
    <style>
        ...
        #app { font-size: 14pt; font-weight: normal; font-family: "Times New Roman", Times,
            serif; color: #222; margin: 0 auto; padding: 0; box-sizing: border-box; background-color: #fff; width:
                19cm; }
        ...
        #page_header { position: sticky; background-color: #f7f7f7; top: 0; z-index: 5; text-align: center;
            border-bottom: 1px solid #ccc; height: 4em; }
        ...
        <div id="page_header">
            <div id="boxes">
                <div id="box_previous" style="display:table">
                    <div style="display:table-cell"><?php if (!empty($nav['firstId'])): ?><a title="<?=
                        h($nav['firstTitle'] ?: 'Untitled') ?>" href="/pages/<?= $nav['firstId'] ?>/print/<?=
                            urlencode($nav['firstTitle'] ?? '') ?>">|&laquo;</a><?php endif; ?></div>
                    <div style="display:table-cell"><?php if (!empty($nav['previousId'])): ?><a title="<?=
                        h($nav['previousTitle'] ?: 'Untitled') ?>" href="/pages/<?= $nav['previousId'] ?>/print/<?=
                            urlencode($nav['previousTitle'] ?? '') ?>">&laquo;</a><?php endif; ?></div>
                </div>
                <div id="box_next" style="display:table">
                    <div style="display:table-cell"><?php if (!empty($nav['nextId'])): ?><a title="<?=
                        h($nav['nextTitle'] ?: 'Untitled') ?>" href="/pages/<?= $nav['nextId'] ?>/print/<?=
                            urlencode($nav['nextTitle'] ?? '') ?>">&raquo;</a><?php endif; ?></div>
                    <div style="display:table-cell"><?php if (!empty($nav['lastId'])): ?><a title="<?=
                        h($nav['lastTitle'] ?: 'Untitled') ?>" href="/pages/<?= $nav['lastId'] ?>/print/<?=
                            urlencode($nav['lastTitle'] ?? '') ?>">&raquo;|</a><?php endif; ?></div>
                </div>
            </div>
        </div>
        <div>
            ...
            <?php if ($showAuthor): ?>
                <span style="line-height:1.5em;display:block;padding:0 1em;border-top:1px solid #ccc;"><?= __('Last
                    update') ?>: <?= $page->modified ? $page->modified->format('d.m.Y H:i') : '' ?> | <?=
                        h($page->modifier->fullname ?? '') ?></span>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</div>
